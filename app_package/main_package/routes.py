...
from app_package.main_package.forms import PostForm
...
from app_package.main_package import bp
from app_package.main_package.forms import SearchForm

# @app.route('/translate', methods=['POST'])
@bp.route('/translate', methods=['POST'])
@login_required
def translate_text():
    return {'text': translate("text",'en','uk')}
...
def before_request():
    if current_user.is_authenticated:
        ...
        # we do not need to call db.session.add(current_user) because current_user is already in the session done by the Flask-Login extension (just as we called current_user)
        db.session.commit()
